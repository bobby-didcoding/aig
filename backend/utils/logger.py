...
individual logger. Some disadvantages:
 ...
 - errors/exceptions from not-our application (django, GBG client and other external agis) will not include extra
   information that was specified in LoggerAdapter
 - if we would like to keep separate applications (authentication, external_agis, frontend_agi, payment_agi) in logger  
...
class agiLogger(logging.Logger):
    def __init__(self, *args, **kwargs):
        super(agiLogger, self).__init__(*args, **kwargs)
    ...
    def makeRecord(self, name, level, fn, lno, msg, args, exc_info, func=None, extra=None, sinfo=None):
        ...
        return super(agiLogger, self).makeRecord(name, level, fn, lno, msg, args, exc_info, func, extra, sinfo)
...
class agiJsonFormatter(jsonlogger.JsonFormatter):
    def add_fields(self, log_record, record, message_dict):
        super(agiJsonFormatter, self).add_fields(log_record, record, message_dict)

        # Map all shared_extra params to root level
        # @NOTE: we cannot map shared_extra in agiLogger, because not all loggers are agiLogger,
